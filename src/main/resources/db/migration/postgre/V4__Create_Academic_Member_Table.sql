...
CREATE TABLE IF NOT EXISTS academic_member (
    -- Primary Key
    academic_member_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Indexes
CREATE INDEX idx_academic_member_username ON academic_member(username);
CREATE INDEX idx_academic_member_device_id ON academic_member(device_id);
CREATE INDEX idx_academic_member_email ON academic_member(email);
CREATE INDEX idx_academic_member_university_number ON academic_member(university_number);
CREATE INDEX idx_academic_member_academic_year_id ON academic_member(academic_year_id);
CREATE INDEX idx_academic_member_role_id ON academic_member(role_id);
CREATE INDEX idx_academic_member_university_id ON academic_member(university_id);
CREATE INDEX idx_academic_member_soft_delete ON academic_member(soft_delete) WHERE soft_delete = FALSE;
CREATE INDEX idx_academic_member_email_verified ON academic_member(email_verified) WHERE email_verified = TRUE;
